...
$(document).on('click', '.open_modal', function () {
    ...
    $.get(url + '/' + product_id, function (data) {
        ...
        $('#p_product_image').html(data.product_image);
        $('#btn-save').val("update");
        $('#myModal').modal('show');
    })
});
...
$("#btn-save").click(function (e) {
    ...
    var formData = {
        ...
        type: $('#type').val(),
        price: $('#price').val(),
        product_image: $('input[type=file]').val().split('\\').pop()
    }
//used to determine the http verb to use [add=POST], [update=PUT]
    ...
    $.ajax({
        ...
        success: function (data) {
            console.log(data);
            var product = '<tr id="product' + data.id + '"><td>' + data.id + '</td><td>' + data.name + '</td><td>'
                    + data.type + '</td><td>' + data.price + '</td><td>' + data.product_image + '</td>';
